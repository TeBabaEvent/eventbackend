...
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #0a0a0f; color: #ffffff;">
    ...
    async def send_pending_bank_transfer_reservation(
        self,
        to_email: str,
        customer_name: str,
        order: Order
        ...
        """
        Envoie un email de réservation en attente de paiement par virement bancaire.
        Inclut les coordonnées bancaires et la communication structurée.
        ...
        Args:
            ...
            order: Commande en attente
        """
        logger.info(f"Envoi email réservation virement bancaire à {to_email}")
        ...
        # Coordonnées bancaires de l'événement
        bank_iban = order.event.bank_account_iban or "Non configuré"
        bank_name = order.event.bank_account_name or "Non configuré"
        bank_bic = order.event.bank_account_bic or ""

        # Communication structurée = numéro de commande
        ...
        # Montant formaté
        amount_formatted = f"{order.amount / 100:.2f}"
...
<head>
    <meta charset="UTF-8">
    <meta name="viewport" content="width=device-width, initial-scale=1.0">
    <title>Réservation par virement - BABA Event</title>
</head>
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, 'Helvetica Neue', Arial, sans-serif; background-color: #0a0a0f; color: #ffffff;">
    <!-- Wrapper -->
    <table width="100%" cellpadding="0" cellspacing="0" style="background-color: #0a0a0f;">
            ...
            <td align="center" style="padding: 20px 10px;">
                ...
                <table width="100%" cellpadding="0" cellspacing="0" style="max-width: 600px; background-color: #0a0a0f;">
                    ...
                    <tr>
                        <td style="padding: 20px 30px; text-align: center;">
                            <h2 style="color: #ffffff; margin: 0 0 12px; font-size: 26px; font-weight: 700;">
                                Réservation en attente de virement
                            </h2>
                            <p style="color: #909090; font-size: 15px; line-height: 1.6; margin: 0;">
                                Bonjour {customer_name},<br>
                                Votre réservation a bien été enregistrée.
                    ...
                    <tr>
                        <td style="padding: 0 30px 20px;">
                            <table width="100%" cellpadding="0" cellspacing="0" style="background: #0f1a2e; border: 1px solid #1a3a5c; border-radius: 12px;">
                                <tr>
                                    <td style="padding: 24px;">
                                        <p style="color: #60a5fa; margin: 0 0 15px; font-size: 16px; font-weight: 700;">
                                            🏦 Coordonnées bancaires
                                        </p>

                                        <table width="100%" cellpadding="0" cellspacing="0">
                                            <tr>
                                                <td style="padding: 10px 0; border-bottom: 1px solid #1a3a5c;">
                                                    <span style="color: #6b7280; font-size: 12px; text-transform: uppercase; letter-spacing: 0.5px;">Bénéficiaire</span><br>
                                                    <span style="color: #fff; font-size: 15px; font-weight: 600;">{bank_name}</span>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td style="padding: 10px 0; border-bottom: 1px solid #1a3a5c;">
                                                    <span style="color: #6b7280; font-size: 12px; text-transform: uppercase; letter-spacing: 0.5px;">IBAN</span><br>
                                                    <span style="color: #fff; font-size: 15px; font-weight: 600; font-family: 'Courier New', Consolas, monospace; letter-spacing: 1px;">{bank_iban}</span>
                                                </td>
                                            </tr>
                                            {"<tr><td style='padding: 10px 0; border-bottom: 1px solid #1a3a5c;'><span style='color: #6b7280; font-size: 12px; text-transform: uppercase; letter-spacing: 0.5px;'>BIC/SWIFT</span><br><span style='color: #fff; font-size: 15px; font-weight: 600;'>" + bank_bic + "</span></td></tr>" if bank_bic else ""}
                                            <tr>
                                                <td style="padding: 10px 0; border-bottom: 1px solid #1a3a5c;">
                                                    <span style="color: #6b7280; font-size: 12px; text-transform: uppercase; letter-spacing: 0.5px;">Montant à virer</span><br>
                                                    <span style="color: #60a5fa; font-size: 22px; font-weight: 800;">{amount_formatted} €</span>
                                                </td>
                                            </tr>
                                            <tr>
                                                <td style="padding: 10px 0;">
                                                    <span style="color: #6b7280; font-size: 12px; text-transform: uppercase; letter-spacing: 0.5px;">Communication</span><br>
                                                    <span style="color: #fbbf24; font-size: 18px; font-weight: 700; font-family: 'Courier New', Consolas, monospace;">{communication}</span>
                                                </td>
                                            </tr>
                                        </table>
                                    </td>
                            ...
                            <table width="100%" cellpadding="0" cellspacing="0" style="background: #2d2510; border: 1px solid #4a3d1a; border-radius: 12px;">
                                <tr>
                                    <td style="padding: 20px 24px;">
                                        ...
                                        <p style="color: #a0a0a0; margin: 0; font-size: 14px; line-height: 1.7;">
                                            Vos billets seront envoyés <strong style="color: #fff;">après réception du virement</strong>.<br><br>
                                            <strong style="color: #fbbf24;">N'oubliez pas</strong> d'indiquer la communication <strong style="color: #fff;">{communication}</strong> lors de votre virement pour que nous puissions identifier votre paiement.
                                        </p>
                                    </td>
                                </tr>
                            </table>
                                    ...
                                    <td style="padding: 18px 24px; background: #1a1a1e; border-bottom: 1px solid #2a2a2e;">
                                        <table width="100%" cellpadding="0" cellspacing="0">
                                            <tr>
                                                ...
                                                <td style="text-align: right;">
                                                    <span style="background: #1a2a4a; color: #60a5fa; padding: 5px 14px; border-radius: 20px; font-size: 11px; font-weight: 700; text-transform: uppercase; letter-spacing: 0.5px;">Virement en attente</span>
                                                </td>
                                            </tr>
                                        </table>
                                    </td>
        ...
        msg = self._create_message(
            to_email=to_email,
            subject=f"Virement bancaire - {order.event.title}",
            html_content=html,
            embedded_images=embedded_images if embedded_images else None
        )
...
async def send_pending_bank_transfer_reservation_email(to_email, customer_name, order):
    """Helper pour envoyer un email de réservation par virement bancaire"""
    await email_service.send_pending_bank_transfer_reservation(to_email, customer_name, order)
